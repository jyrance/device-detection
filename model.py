...
def draw_boxes(image, boxes, class_names, scores, max_boxes=10, min_score=0.1):
    ...
    for i in range(min(boxes.shape[0], max_boxes)):
        devices = ['Casette deck', 'Headphone', 'Computer keyboard', 'Printer', 'Computer mouse', 'Calculator', 'Computer monitor', 'Mobile phone', 'Television', 'Telephone', 'Tablet computer', 'Camera', 'Corded phone']
        if class_names[i].decode('ascii') not in devices:
            continue
...
def load_model():
    ...
    print('Loading mobilenet model from TF hub...')
...
def run_detector(detector, img):
    ...
    # print(base64.b64encode(image_with_boxes))
    # display_image(image_with_boxes)
    return [image_with_boxes, end_time-start_time]
